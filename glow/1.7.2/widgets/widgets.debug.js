...
(window.gloader || glow).module({
	name: "glow.widgets",
	library: ["glow", "1.7.2"],
	depends: [["glow", "1.7.2", 'glow.dom', 'glow.events']],
	builder: function(glow) {
		...
		glow.ready(function() {
			...
			var testDiv = glow.dom.create('<div class="glow172-cssTest" style="height:0;position:absolute;visibility:hidden;top:-20px;display:block"></div>').appendTo(docBody);
			
			// not testing for height as that break in some browsers' 'zoom' implementations
			if (testDiv.css("visibility") != 'hidden') {
				//css disabled
				docBody.className += " glow172-basic";
			} else {
...
(window.gloader || glow).module({
	...
	builder: function(glow) {
		var dom = glow.dom,
			...
			*/
			overlayHtml = '<div class="glow172-overlay glowNoMask"><div class="overlay-focalPoint" tabindex="-1"></div></div>',
		...
		*/
		function Overlay(content, opts) {
			...
			this.opts = opts = glow.lang.apply({
				...
				focusOnShow: false,
				id: "glow172Overlay" + (++overlayCount),
				closeOnEsc: false
			}, opts);
	...
	builder: function(glow) {
			...
			glow.ready(function () {
				var widgetSheet = function (sheets) {
					...
					_ieCssRule = function(theme, className, image) {
						return ".glow172-ie .glow172-overlay" + cssPngThemes[theme].className + " ." +
		...
		*/
		defaultTemplate = function() {
			var r = [], rLen = 0;
			r[rLen++] = '<div class="glow172-panel">';
		...
		function Panel(content, opts) {
			...
			*/
			...
			if (!customTemplate) {
				...
				if (!themesPreloaded[opts.theme] && docBody.className.indexOf("glow172-basic") == -1) {
					fullContentClone = fullContent.clone().addClass("glow172-panel-preload").appendTo(docBody);
...
(window.gloader || glow).module({
	...
	builder: function(glow) {
		var dom = glow.dom,
			...
			env = glow.env,
			win,
			positionRegex = /glow172\-infoPanel\-point[TRBL]/,
		...
		*/
		function InfoPanel(content, opts) {
			...
			if (!customTemplate) {
				this.content.addClass("glow172-infoPanel");
			}

			this.content.addClass("glow172-infoPanel-point" + (opts.pointerPosition || "t").slice(0,1).toUpperCase());
		}
			...
			setPosition: function(x, y) {
				...
				if (!pointerPosition) {
					...
					if (lastPointerPosition != pointerPosition) {
						...
						contentNode.className = contentNode.className.replace(positionRegex, "glow172-infoPanel-point" + pointerPosition);
...
(window.gloader || glow).module({
	...
	builder: function(glow) {
		...
		var $ = glow.dom.get,
			...
			/*
			 HTML for the slider 
			 This still needs to be wrapped in <div class="glow172-slider"> or
			 <div class="glow172-vSlider"> before it's created
			*/
			SLIDER_TEMPLATE = '' +
		...
		(function() {
			...
			*/
			Ruler = function(container, opts) {
				...
				// create element
				this.element = element = glow.dom.create('<div role="presentation" class="glow172-' + vocab.containerClassNamePart + '"><div class="ruler-spacer"></div><div class="ruler-labels"></div></div>');
		...
		*/
		...
		Slider = glow.widgets.Slider = function(container, opts) {
			...
			*/
			...
			this.element = element = glow.dom.create('<div class="glow172-' + vocab.containerClassNamePart + '" tabindex="0" role="slider" aria-disabled="false">' + SLIDER_TEMPLATE + '</div>');
...
/* private functions **********************************************************/
		...
		function getParentListItem(that, node) { /*debug*///console.log("getParentListItem("+node+")");
			...
			while (listItem.parentNode && listItem.parentNode.parentNode) {
				if ($(listItem.parentNode.parentNode).hasClass('glow172-autoSuggest')) break;
		...
		*/
		glow.widgets.AutoSuggest = function(inputElement, dataSource, opts) { /*debug*///console.log('new glow.widgets.AutoSuggest('+inputElement+', '+dataSource+', '+opts+')');
			...
			this.overlay = new glow.widgets.Overlay(
				glow.dom.create('<div class="glow172-autoSuggest"><ul></ul></div>'),
				{
					autoPosition: false,
...
/* public methods *************************************************************/
		...
		glow.widgets.AutoSuggest.prototype.configure = function(opts) {
			...
			if (this.opts.height) {
				var listContainer = $(this.overlay.container.get('.glow172-autoSuggest').get('ul')[0]);
		...
		function Carousel(container, opts) {
			...
			this.element =
				dom.create("<div"
					...
					+ " class=\"" + (this._opts.vertical? "glow172-vCarousel" : "glow172-carousel")
			...
			if (this._opts.vertical) {
				this.element.addClass("glow172-vCarousel");
			}
			else {
				this.element.addClass("glow172-carousel");
			}
	...
	builder: function(glow) {
		...
		*/
		...
		glow.widgets.Editor = function(textarea, opts) {
			...
			// interpolate context
			this.element = glow.dom.create('<div class="glow172-editor"><p class="glow172-hidden">{ENTER_MESSAGE}, <a href="#endOfEditor' + endOfEditorCounter() + '" tabindex="0">{SKIP_LINK_TEXT}</a></p><div class="editor-' + (opts.theme || "light") + '"><div class="editor-state"></div></div><p id="endOfEditor' + endOfEditorCounter() + '" class="glow172-hidden endOfEditorCounter" tabindex="0">{LEAVE_MESSAGE}</p></div>', {interpolate : editorLocaleModule});
